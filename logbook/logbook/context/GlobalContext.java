...
import logbook.context.room.BattlePracticeRoom;
import logbook.context.room.BattleRoom;
import logbook.context.room.CreateItemRoom;
import logbook.context.room.CreateShipRoom;
import logbook.context.room.DeckRoom;
import logbook.context.room.DestroyItemRoom;
import logbook.context.room.DestroyShipRoom;
import logbook.context.room.HokyoRoom;
import logbook.context.room.KaisouRoom;
import logbook.context.room.MainRoom;
import logbook.context.room.MissionRoom;
import logbook.context.room.NyukyoRoom;
import logbook.context.room.PracticeRoom;
import logbook.context.room.QuestRoom;
import logbook.context.room.RemodelRoom;
...
public class GlobalContext {
	...
	/** 远征记录 */
	private final static List<MissionResultDto> missionResulutList = new ArrayList<>();
	/** 开发记录 */
	...
	/** masterdata */
	private static MasterDataDto masterData;

	/** 更新GlobalContext  */
	public static void updateContext(DataType type, Data data, String serverName) {
		GlobalContext.serverName = serverName;

		int api_result = data.getJsonObject().getInt("api_result");
		if (api_result != 1) {
			ApplicationMain.main.logPrint("猫了,错误码:" + api_result);
			return;
		}
		...
		switch (type) {
			...
			case BATTLE_START_AIR_BASE:
				...
				break;
			//end
			//start practiceBattleRoom
			case BATTLE_PRACTICE_DAY:
				practiceBattleRoom.doPracticeBattleDay(data, json);
				break;
			case BATTLE_PRACTICE_MIDNIGHT:
				practiceBattleRoom.doPracticeBattleMidnight(data, json);
				break;
			case BATTLE_PRACTICE_RESULT:
				practiceBattleRoom.doPracticeBattleResult(data, json);
				break;
			//end
	...
	public static void setMaterial(int[] mm) {
		if (currentMaterial == null || mm == null) return;
	...
	public static void destroyShip(long time, String event, int id) {
		...
		if (ship != null) {
			final int count = ShipTranslator.getSlotCount(ship);
	...
	public static List<MissionResultDto> getMissionresulutlist() {
		return missionResulutList;
	}
	...
	private static final BattlePracticeRoom practiceBattleRoom = new BattlePracticeRoom();

	public static BattleRoom getBattleroom() {
		return battleRoom;
	}

	public static BattlePracticeRoom getPracticebattleroom() {
		return practiceBattleRoom;
	}
	...
	public static class BattleList {
		...
		public BattleDto last() {
			BattleDto res = this.last;
			this.last = null;
			return res;
		}
	}

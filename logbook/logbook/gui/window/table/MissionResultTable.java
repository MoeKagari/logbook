...
public class MissionResultTable extends AbstractTable<MissionResultDto> {
	...
	protected void initTCMS(List<TableColumnManager> tcms) {
		...
		tcms.add(new TableColumnManager("道具1", rd -> ToolUtils.notNull(rd.getItems()[0], MissionResultItem::getName, "")));
		tcms.add(new TableColumnManager("数量", true, rd -> ToolUtils.notNull(rd.getItems()[0], MissionResultItem::getCount, "")));
		tcms.add(new TableColumnManager("道具2", rd -> ToolUtils.notNull(rd.getItems()[1], MissionResultItem::getName, "")));
		tcms.add(new TableColumnManager("数量", true, rd -> ToolUtils.notNull(rd.getItems()[1], MissionResultItem::getCount, "")));
	}
